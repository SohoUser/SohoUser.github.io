...
import LazyInput from '~com/input/input';
...
export default class extends React.PureComponent {
  ...
  render() {
    return (
      <div>
        <button ref={this.minBtn} onClick={this.decrease}>-</button>
        <LazyInput
          nativeProps={{ type: 'text', min: this.props.max, max: this.props.max, type: 'number'}}
          value={this.props.cnt}
          onChange={this.onChange}
          ref={this.lazyInput}
        />
        <button ref={this.maxBtn} onClick={this.increase}>+</button>
      </div>
    );
  }
}
